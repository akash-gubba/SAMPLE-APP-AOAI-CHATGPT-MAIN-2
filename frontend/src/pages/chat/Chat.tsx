...
// if (iframe) {
//   // Set the onload event handler before accessing contentWindow or the document
//   console.log("yes");
  
//   iframe.onload = function () {
//     const iframeDoc = iframe.contentWindow ? iframe.contentWindow.document : null;

//     if (iframeDoc) {
//       const questionInput = iframeDoc.querySelector('textarea[aria-labelledby="QuestionId_ra24ba97ca3074d65b627bb07b93ab831 QuestionInfo_ra24ba97ca3074d65b627bb07b93ab831"]') as HTMLTextAreaElement
//       const answerinput = iframeDoc.querySelector('textarea[aria-labelledby="QuestionId_rb10dc1bf65d44e6b956dc0816a94d1a3 QuestionInfo_rb10dc1bf65d44e6b956dc0816a94d1a3"]') as HTMLTextAreaElement

//       if (questionInput && answerinput) {
//         questionInput.textContent = "what is sdx";
//         answerinput.textContent = 'solution tosdx';
//       }
//     }
//   };
// }
...
// const formUrl = `https://forms.office.com/e/SjVASf2pcu?embed=true&question=${encodedQuestion}&conversationid=${encodedConversationId}&response=${encodedResponse}`;
const Chat = () => {
  ...
  const makeApiRequestWithoutCosmosDB = async (question: string, conversationId?: string, fileContent?:string |null) => {
    setIsLoading(true)
    if (question){
    ...
    const request: ConversationRequest = {
      ...
      fileContent: fileContent || null, ///akash: Added the file content
    }
  ...
  const makeApiRequestWithCosmosDB = async (question: string, conversationId?: string,fileContent?:string | null) => {
    console.log(conversationId)
    setIsLoading(true)
    setShowLoadingMessage(true)
    setIsLoading(true)
      ...
      {showAuthMessage ? (
        ...
        <Stack horizontal className={styles.chatRoot}>
          <div className={styles.chatContainer}>
                    ...
                    {answer.role === 'user' ? (
                      ...
                      <div className={styles.chatMessageGpt}>
                        <Answer
                          ...
                          onconvid={(id: SetStateAction<string>)=>setconvid(id)}
                          onCitationClicked={c => onShowCitation(c)}
                          onExectResultClicked={() => onShowExecResult()}
                          onShowIframeChange={handleShowIframeChange}
                        />
                      </div>
                    ) : answer.role === ERROR ? (
            ...
            <Stack horizontal className={styles.chatInput}>
              ...
              <QuestionInput
                ...
                disabled={isLoading}
                onSend={(question, id,fileContent) => {
                  appStateContext?.state.isCosmosDBAvailable?.cosmosDB
                    ? makeApiRequestWithCosmosDB(question, id,fileContent)
                    : makeApiRequestWithoutCosmosDB(question, id,fileContent)
                }}
                conversationId={
          ...
          <Stack.Item className={styles.citationPanel} tabIndex={0} role="tabpanel" aria-label="Citations Panel">
            ...
            <div tabIndex={0} className={styles.citationPanelContent} style={{ height: '100%', width: '100%' }}>
              <iframe
                className='iframe'
                width="640px"
                height="800px"
                src={`https://forms.office.com/Pages/ResponsePage.aspx?id=PqsWG_a440-fPi23_lSfak5RWon8k2FKj6z1XAeBP4ZUNE5LU1gwMjA0RFFPTVlQRjBPT1NCVTM4TC4u&ra24ba97ca3074d65b627bb07b93ab831=${formquestion}&r4a5950a71bcc4961a09c690c31803231=${convid}`}
                style={{ border: 'none', maxWidth: '100%', maxHeight: '100%' }}
                aria-label="Microsoft Form embedded for user feedback"
                allowFullScreen/>
              {/* </iframe> */}
            </div>
          </Stack.Item>)}
