...
import { Checkbox, DefaultButton, Dialog, FontIcon, IconButton, Stack, Text } from '@fluentui/react'
...
interface Props {
  ...
  onShowIframeChange: () => void;
}

export const Answer = ({ answer,onconvid, onCitationClicked, onExectResultClicked,onShowIframeChange }: Props) => {
  ...
  // useEffect(() => {
  //   onShowIframeChange(); // Notify parent component of the iframe visibility state
  // }, [showIframe, onShowIframeChange]);
  ...
  const onDislikeResponseClicked = async () => {
    ...
    if (feedbackState === undefined || feedbackState === Feedback.Neutral || feedbackState === Feedback.Positive) {
      ...
      // onShowIframeChange() // Show the iframe when dislike is clicked
    } else {
  ...
  const resetFeedbackDialog = () => {
    ...
    // setShowIframe(false) // Hide the iframe when dialog is closed
  }
      ...
      <>
        ...
        <Stack tokens={{ childrenGap: 4 }}>
          ...
          <Checkbox
            ...
            id={Feedback.IncompleteAnswer}
            defaultChecked={negativeFeedbackList.includes(Feedback.IncompleteAnswer)}
            onChange={updateFeedbackList}></Checkbox>
            <Checkbox
            label="Not answering to the point"
            id={Feedback.NAT}
            defaultChecked={negativeFeedbackList.includes(Feedback.NAT)}
            onChange={updateFeedbackList}></Checkbox>
            <Checkbox
            label="Too much additional information"
            id={Feedback.AddInfo}
            defaultChecked={negativeFeedbackList.includes(Feedback.AddInfo)}
            onChange={updateFeedbackList}></Checkbox>
            <Checkbox
            label="was too technical while i expected a technical_response"
            id={Feedback.techfunc}
            defaultChecked={negativeFeedbackList.includes(Feedback.techfunc)}
            onChange={updateFeedbackList}></Checkbox>
            <Checkbox
            label="was too functional while i expected a functional response"
            id={Feedback.functech}
            defaultChecked={negativeFeedbackList.includes(Feedback.functech)}
            onChange={updateFeedbackList}></Checkbox>
            <Checkbox
            label="References are not provided / wrong / not working"
            id={Feedback.Noref}
            defaultChecked={negativeFeedbackList.includes(Feedback.Noref)}
            onChange={updateFeedbackList}></Checkbox>
            <Checkbox
            label="The response is not related to hexagon products answer"
            id={Feedback.notRelated}
            defaultChecked={negativeFeedbackList.includes(Feedback.notRelated)}
            onChange={updateFeedbackList}></Checkbox>

        </Stack>
        <div onClick={() => showfedback()} style={{ color: '#115EA3', cursor: 'pointer' }}>
        I have additional content to improve responses      {/*Akash*/}
        </div>
      </>
    )
  }
